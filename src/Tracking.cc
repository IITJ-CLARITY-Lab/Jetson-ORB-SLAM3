...
*
* Modified 2026 by CLARITY Lab, Indian Institute of Technology Jodhpur.
* Changes: CUDA GPU ORB feature extraction reproducing the reference detector,
* a pipelined front end overlapping extraction with tracking, and CNN-based
* loop-closure candidate retrieval through TensorRT.
*
...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    // Front-end selection: GPU by default; env CPU_ORB=1 builds the unmodified CPU
    // extractor instead, so the GPU-vs-CPU equivalence A/B runs from one binary
    // (it previously required editing this line and rebuilding).
    const bool kUseGpuORB = (getenv("CPU_ORB") == nullptr);
    if(!kUseGpuORB)
        std::cout << "[ORB] CPU_ORB set: using reference CPU extractor" << std::endl;

    mpORBextractorLeft = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST, kUseGpuORB, 350, 600);

    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST, kUseGpuORB, 350, 600);

    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
        mpIniORBextractor = new ORBextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST, kUseGpuORB, 350, 600);

    // economy front-end config (env-tunable: AFE_ECO_NFEAT / AFE_ECO_SCALE / AFE_ECO_NLEVELS)
    int ecoNF = getenv("AFE_ECO_NFEAT") ? atoi(getenv("AFE_ECO_NFEAT")) : 800;
    float ecoSF = getenv("AFE_ECO_SCALE") ? (float)atof(getenv("AFE_ECO_SCALE")) : 1.5f;
    int ecoNL = getenv("AFE_ECO_NLEVELS") ? atoi(getenv("AFE_ECO_NLEVELS")) : 4;
    mpORBextractorEcoLeft = new ORBextractor(ecoNF, ecoSF, ecoNL, fIniThFAST, fMinThFAST, kUseGpuORB, 350, 600);
    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
        mpORBextractorEcoRight = new ORBextractor(ecoNF, ecoSF, ecoNL, fIniThFAST, fMinThFAST, kUseGpuORB, 350, 600);

    // pipelined front-end worker pair (CPU: own pyramids, no GPU contention)
    if(getenv("PIPELINE_FE") && (mSensor==System::STEREO || mSensor==System::IMU_STEREO) && !PrefetchFE::I().eL){
        PrefetchFE::I().eL = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,false,350,600);
        PrefetchFE::I().eR = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,false,350,600);
    }
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    // --- Pipelined front-end (env PIPELINE_FE): track frame t-1 while extracting frame t ---
    cv::Mat useL = mImGray, useR = imGrayRight; double useTs = timestamp;
    static const bool kPipeFE = (getenv("PIPELINE_FE") != nullptr);
    if(kPipeFE)
    {
        PrefetchFE &P = PrefetchFE::I();
        if(P.fut.valid()) P.fut.wait();
        const bool havePrev = P.pendingValid;
        if(havePrev){ P.promote(); useL = P.pimL; useR = P.pimR; useTs = P.pts; }
        P.pimL = mImGray.clone(); P.pimR = imGrayRight.clone(); P.pts = timestamp; P.pendingValid = true;
        P.armed = true;
        if(!havePrev)
        {
            // first frame: nothing to track yet; extract it and return
            P.fut = std::async(std::launch::async, [&P](){
                std::vector<int> vLap = {0,0};
                std::thread tR([&P,&vLap](){ P.monoR = (*P.eR)(P.pimR, cv::Mat(), P.kR, P.dR, vLap); });
                P.monoL = (*P.eL)(P.pimL, cv::Mat(), P.kL, P.dL, vLap);
                tR.join();
            });
            P.armed = false;
            return Sophus::SE3f();
    ...
    // --- IMU-adaptive front-end selection (env ADAPTIVE_FE) ---
    ORBextractor* pAFEL = mpORBextractorLeft;
    ORBextractor* pAFER = mpORBextractorRight;
    {
        static const bool kAdaptiveFE = (getenv("ADAPTIVE_FE") != nullptr);
        if(kAdaptiveFE && mpORBextractorEcoLeft && (mSensor==System::IMU_STEREO))
        {
            float wsum = 0.f; int nw = 0;
            {
                std::unique_lock<std::mutex> lock(mMutexImuQueue);
                for(std::list<IMU::Point>::const_iterator it=mlQueueImuData.begin(); it!=mlQueueImuData.end(); ++it){ wsum += it->w.norm(); ++nw; }
            }
            const float wmag = nw ? wsum/nw : 1e9f;   // no IMU yet -> stay FULL
            static const float kCalm = getenv("AFE_CALM") ? atof(getenv("AFE_CALM")) : 0.35f;
            static const float kAggr = getenv("AFE_AGGR") ? atof(getenv("AFE_AGGR")) : 0.60f;
            if(mbEcoMode && wmag > kAggr) mbEcoMode = false;
            else if(!mbEcoMode && wmag < kCalm) mbEcoMode = true;
            if(mbEcoMode){ pAFEL = mpORBextractorEcoLeft; pAFER = mpORBextractorEcoRight; }
            static long afeCnt=0, afeEco=0; ++afeCnt; if(mbEcoMode) ++afeEco;
            if(afeCnt%500==0) std::cout << "[AdaptiveFE] frames=" << afeCnt << " eco_frac=" << (double)afeEco/afeCnt << " wmag=" << wmag << std::endl;
        }
    }

    if(kPipeFE){ pAFEL = PrefetchFE::I().eL; pAFER = PrefetchFE::I().eR; }
    if (mSensor == System::STEREO && !mpCamera2)
        mCurrentFrame = Frame(useL,useR,useTs,pAFEL,pAFER,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::STEREO && mpCamera2)
        mCurrentFrame = Frame(useL,useR,useTs,pAFEL,pAFER,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr);
    else if(mSensor == System::IMU_STEREO && !mpCamera2)
        mCurrentFrame = Frame(useL,useR,useTs,pAFEL,pAFER,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    else if(mSensor == System::IMU_STEREO && mpCamera2)
        mCurrentFrame = Frame(useL,useR,useTs,pAFEL,pAFER,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr,&mLastFrame,*mpImuCalib);

    //cout << "Incoming frame ended" << endl;

    if(kPipeFE && PrefetchFE::I().armed)
    {
        PrefetchFE &P = PrefetchFE::I();
        P.armed = false;
        P.fut = std::async(std::launch::async, [&P](){
...
void Tracking::CreateNewKeyFrame()
{
    ...
    pKF->mImGray = mImGray.clone();  // for CNN loop closure
