...
void System::SaveTrajectoryTUM(const string &filename)
{
    ...
    // Save the biggest map (as SaveTrajectoryEuRoC does): a tracking loss near the
    // end leaves a fresh, possibly empty ACTIVE map, which is not the trajectory.
    ...
    if(!pBiggerMap)
    {
        cerr << "ERROR: SaveTrajectoryTUM: no keyframes in atlas." << endl;
        return;
    }
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        // Heavy culling (e.g. a static camera) can break the chain at a null parent — skip the frame
...
{
    // Dependency-free FNV-1a hash. This is an integrity hint for atlas
    // save/load, not a security feature; openssl dev libs are absent on
    // stock Jetson images, so MD5 is deliberately avoided.
    string checksum = "";
    ...
    if ( !f.is_open() )
    {
        cout << "[E] Unable to open the in file " << filename << " for checksum." << endl;
        return checksum;
    }
