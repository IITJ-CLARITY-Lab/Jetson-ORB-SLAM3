...
#endif
    // GPU ORB uses shared static buffers — serialize for thread safety
    ExtractORB(0,imLeft,0,0);
    ExtractORB(1,imRight,0,0);
    // threadLeft.join(); // serialized above
    // threadRight.join(); // serialized above
#ifdef REGISTER_TIMES
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1)
{
    ...
    if(flag==0){
        if(PrefetchFE::I().hasL) PrefetchFE::I().takeL(mvKeys, mDescriptors, monoLeft);
        else monoLeft = (*mpORBextractorLeft)(im,cv::Mat(),mvKeys,mDescriptors,vLapping);
    } else {
        if(PrefetchFE::I().hasR) PrefetchFE::I().takeR(mvKeysRight, mDescriptorsRight, monoRight);
