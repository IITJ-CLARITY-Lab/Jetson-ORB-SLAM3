...
namespace ORB_SLAM3 {
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        // For Rectified stereo (e.g. KITTI) camera 2 shares camera 1's intrinsics and
        // originalCalib2_ is never allocated in readCamera2 — guard against the null deref.
        if((settings.sensor_ == System::STEREO || settings.sensor_ == System::IMU_STEREO) && settings.originalCalib2_){
