    ...
    mbLoopDetected(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mbActiveLC(bActiveLC)
{
    ...
    // Initialize CNN loop detector (CosPlace)
    mpCNNDetector = nullptr;
    mfCNNSimThreshold = 0.85f;  // default; overridden via SetCNNSimThreshold()
    try {
        mpCNNDetector = new ORB_SLAM3::CNNLoopDetector("cosplace_r50_512.onnx");
        // The detector reports a missing engine on stderr and returns inert
        // rather than throwing, so ask it before claiming success — otherwise
        // this prints "initialized" directly under "engine not found".
        if(mpCNNDetector->IsReady())
            cout << "[CNN] Loop detector initialized" << endl;
        else {
            delete mpCNNDetector;
            mpCNNDetector = nullptr;
            cout << "[CNN] No TensorRT engine; loop closure uses DBoW2 only" << endl;
        }
    } catch(...) {
        delete mpCNNDetector;
        mpCNNDetector = nullptr;
        cout << "[CNN] Failed to load model, using DBoW2 only" << endl;
    }
            ...
            if(bFindedRegion)
            {
                ...
                if(mbLoopDetected)
                {
                    ...
                    if(mpCurrentKF->GetMap()->IsInertial())
                    {
                        ...
                        // Adaptive gravity-aware loop acceptance: the stock guard
                        // rejects loops whose implied roll/pitch (tilt) correction
                        // exceeds ~0.46 deg; under aggressive inertial motion that
                        // tilt is accumulated gravity drift, not a false loop.
                        // Accept large-tilt loops as YAW-ONLY corrections, gated by
                        // a stricter inlier threshold. Env ADAPTIVE_LC toggles A/B.
                        double tilt = std::max(fabs(phi(0)), fabs(phi(1)));
                        int nLoopInliers = 0;
                        for(size_t i=0;i<mvpLoopMatchedMPs.size();++i)
                            if(mvpLoopMatchedMPs[i]) nLoopInliers++;
                        const int kInliersStrict = 40;
                        bool yawOK     = fabs(phi(2)) < 0.349f;
                        bool tiltSmall = (tilt < 0.008);
                        static const bool kAdaptiveLC = (getenv("ADAPTIVE_LC") != nullptr);
                        const double kTiltCap = 0.05;    // v2: bounded recovery window (rad)
                        bool accept    = kAdaptiveLC
                            ? (yawOK && (tiltSmall || (nLoopInliers >= kInliersStrict && tilt < kTiltCap)))
                            : (yawOK && tiltSmall);
                        cout << "[GuardEval] tilt=" << tilt << " yaw=" << phi(2)
                             << " inliers=" << nLoopInliers << " adaptive=" << kAdaptiveLC
                             << " -> " << (accept?"ACCEPT":"REJECT") << endl;
                        if (accept)
                        {
                            if(mpCurrentKF->GetMap()->IsInertial())
                            {
                        ...
                        {
                            cout << "BAD LOOP!!! tilt=" << tilt << " yaw=" << phi(2) << " inliers=" << nLoopInliers << endl;
                            bGoodLoop = false;
                        }
...
bool LoopClosing::NewDetectCommonRegions()
{
    ...
    if(!bMergeDetectedInKF || !bLoopDetectedInKF)
    {
        ...
        // CNN loop candidates with two-stage verification:
        // Stage 1: CNN global descriptor similarity (coarse filter, adaptive threshold)
        // Stage 2: ORB local feature matching (fine filter, rejects perceptual aliasing)
        if(mpCNNDetector && mbCNNEnabled)
        {
            mpCNNDetector->AddKeyFrame(mpCurrentKF);
            vector<KeyFrame*> vpCNNCand = mpCNNDetector->DetectLoopCandidates(
                mpCurrentKF, 5, 50, mfCNNSimThreshold);
            ORBmatcher matcherCNN(0.75, true);
            for(auto pKF : vpCNNCand)
            {
                if(pKF->isBad()) continue;
                // Skip if already in BoW candidates
                bool bDup = false;
                for(auto pBow : vpLoopBowCand)
                    if(pBow == pKF) { bDup = true; break; }
                if(bDup) continue;
                // Stage 2: require ORB feature overlap (>=10 matches) to avoid
                // perceptual aliasing false positives from CNN
                vector<MapPoint*> vpMatchedMPs;
                int nmatches = matcherCNN.SearchByBoW(mpCurrentKF, pKF, vpMatchedMPs);
                if(nmatches >= 10)
