...
public:
    ...
    void SetCNNSimThreshold(float th) { mfCNNSimThreshold = th; }
...
protected:
    ...
    // CNN loop closure (CosPlace, optional — supplements DBoW2)
    ORB_SLAM3::CNNLoopDetector* mpCNNDetector;
    float mfCNNSimThreshold;     // cosine similarity threshold (adaptive baseline)

    // CNN self-calibration counters
    int mnCNNProposed = 0;       // CNN candidates proposed (within window)
    int mnCNNVerified = 0;       // CNN candidates that passed Sim3 verification
    bool mbCNNEnabled = true;    // auto-disabled if fail rate > 80%
    static const int CNN_WINDOW = 10; // evaluation window size
